...
// Thread-local storage for audio callback (since decodeAndPlaySample has no context)
...
static int ar_init(int audio_configuration, const POPUS_MULTISTREAM_CONFIGURATION opus_config, void *context, int ar_flags) {
	...
	current_audio_instance = self; // Save for decodeAndPlaySample
...
static void ar_decode_and_play_sample(char *sample_data, int sample_length) {
	...
	// Limelight always sends stereo 48kHz signed 16-bit PCM
	int sample_count = sample_length / sizeof(int16_t);
	int frame_count = sample_count / 2; // stereo → 2 samples per frame
	...
	self->audio_playback->push_buffer(buffer); // ✅ Now correct type
}
...
MoonlightStream::MoonlightStream() {
	render_context = this;
	MoonlightStream::instance_map[render_context] = this;
}

MoonlightStream::~MoonlightStream() {
	MoonlightStream::instance_map.erase(render_context);
	stop_connection();
}
...
bool MoonlightStream::start_connection() {
	...
	if (video_codec == CODEC_H264) {
		supported_formats |= VIDEO_FORMAT_H264;
	} else if (video_codec == CODEC_HEVC) {
		supported_formats |= enable_hdr ? VIDEO_FORMAT_H265_MAIN10 : VIDEO_FORMAT_H265;
	} else if (video_codec == CODEC_AV1) {
		supported_formats |= enable_hdr ? VIDEO_FORMAT_AV1_MAIN10 : VIDEO_FORMAT_AV1_MAIN8;
	}
	...
	// LiStartConnection has 9 parameters!
